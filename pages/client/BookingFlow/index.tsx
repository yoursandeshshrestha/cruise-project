...
import { BookingState, BookingStep, ParkingType, VehicleType } from '../../../types';
...
import { calculateParkingPrice, getParkingDaysLabel } from '../../../constants';
import { Check, ChevronRight, AlertCircle, Calendar, CreditCard, Lock, Ship, MapPin, Tag, Car, Truck } from 'lucide-react';
...
export const BookingFlow: React.FC = () => {
  ...
  const calculateTotal = () => {
    ...
    const parkingCost = calculateParkingPrice(diffDays, booking.vehicleType);
    ...
    const finalTotal = Math.max(0, parkingCost + addOnsCost - promoDiscount);
  ...
  }, [booking.dropOffDate, booking.returnDate, booking.selectedAddOns, booking.vehicleType, promoDiscount, addOns]);
  ...
  const renderStep2 = () => (
    <div className="space-y-8 animate-in slide-in-from-right duration-300">
      <div>
        <h2 className="text-2xl font-bold text-brand-dark mb-4">Your Parking Service</h2>
        <div className="border-2 border-primary bg-blue-50 rounded-lg p-6 pt-12 md:pt-6 relative">
          ...
          <p className="text-sm text-gray-600 mb-4">Secure parking at our monitored facility with a free shuttle transfer to/from the terminal.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="md:col-span-2">
          <label className="block text-sm font-medium text-gray-700 mb-2">
            <span className="text-red-600">*</span> Vehicle Type
          </label>
          <div className="grid grid-cols-2 gap-3">
            <button
              type="button"
              onClick={() => updateBooking('vehicleType', 'car')}
              className={`py-3 px-4 border-2 rounded-lg flex items-center justify-center gap-2 transition-all cursor-pointer ${
            ...
            <button
              type="button"
              onClick={() => updateBooking('vehicleType', 'van')}
              className={`py-3 px-4 border-2 rounded-lg flex items-center justify-center gap-2 transition-all cursor-pointer ${
          ...
          {booking.vehicleType === 'van' && (
            <p className="text-xs text-amber-600 mt-2">Vans require 1.5 parking spaces. A 40% surcharge applies to the parking cost.</p>
          )}
        </div>
        <Input
                ...
                <div className="space-y-4 text-sm">
                  ...
                  <div className="pt-4 mt-4 space-y-2 border-t border-gray-200">
                    {(() => {
                      ...
                      const carParkingCost = calculateParkingPrice(numberOfDays, 'car');
                      const parkingCost = calculateParkingPrice(numberOfDays, booking.vehicleType);
                      const vanSurcharge = booking.vehicleType === 'van' ? parkingCost - carParkingCost : 0;
                      const daysLabel = getParkingDaysLabel(numberOfDays);
                        ...
                        <>
                          {numberOfDays > 0 && (
                            <div className="flex justify-between text-sm text-gray-600">
                              <span>Parking ({daysLabel})</span>
                              <span>£{carParkingCost.toFixed(2)}</span>
                            </div>
                          )}
                          {vanSurcharge > 0 && (
                            <div className="flex justify-between text-sm text-amber-600">
                              <span>Van surcharge (40%)</span>
                              <span>£{vanSurcharge.toFixed(2)}</span>
                            </div>
                          )}
                          {addOnsCost > 0 && (
                            <div className="flex justify-between text-sm text-gray-600">
