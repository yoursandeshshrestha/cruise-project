...
export const Prices: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetchActiveAddOns();
  }, [fetchPricingRules, fetchActiveAddOns, initialized]);
              ...
              <div className="p-8">
                ...
                <div className="overflow-hidden rounded-lg border border-gray-200">
                  <table className="w-full text-sm">
                      ...
                      <tr className="bg-gray-50">
                        ...
                        <th className="text-right p-3 font-semibold text-amber-700">
                          <span className="flex items-center justify-end gap-1">
                            <Truck size={14} /> Van ({vanMultiplier.toFixed(1)}× car price)
                          </span>
                        </th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {[1, 2, 3, 7, 14].map((days) => {
                        const carPrice = calculateCarPrice(days);
                        const vanPrice = calculateVanPrice(days);
                        const label = days === 1 ? '1 Day' : days === 7 ? '1 Week' : days === 14 ? '2 Weeks' : `${days} Days`;
                ...
                <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                    {[3, 7, 10, 14].map(days => {
                      const price = calculateCarPrice(days);
                      return (
                ...
                <div className="mt-4 flex items-start gap-2 text-xs text-gray-500">
                  ...
                  <p>Van pricing is calculated as total car price × {vanMultiplier.toFixed(1)}, rounded to the nearest pound.</p>
                </div>
            ...
            {addOns.map((addon) => (
              <div key={addon.id} className="bg-white p-6 rounded-xl shadow-light border border-gray-100 hover:shadow-medium transition-shadow">
                ...
                <div className="flex justify-between items-center border-t border-gray-100 pt-4">
                  ...
                  <Link to="/book">
                     <Button variant="secondary" className="px-4 py-2 text-sm cursor-pointer">Add to Booking</Button>
                  </Link>
                </div>
              </div>
            ))}
